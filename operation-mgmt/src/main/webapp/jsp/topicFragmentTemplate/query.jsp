							...
							<div class="panel-body">
								...
								<div class="adv-table">
									<table class="display table table-bordered table-striped" id="dynamic-table">
										...
										<tbody>
											<c:forEach items="${dataList}" var="versionItem" varStatus="status">
												<tr class="gradeX">
													<td>${status.index + 1}</td>
													<td>${versionItem.type }</td>
													<td>
														<c:choose>
	                                                		<c:when test="${versionItem.contenttype == '0'}">
	                                                			文本
	                                                		</c:when>
	                                               			<c:when test="${versionItem.contenttype == '1'}">
	                                               				图片
	                                               			</c:when>
	                                               			<c:when test="${versionItem.contenttype == '2'}">
	                                               				视频
	                                               			</c:when>
	                                               			<c:when test="${versionItem.contenttype == '3'}">
	                                               				音频 
	                                                		</c:when>
	                                                	</c:choose>
													</td>
													...
													<td>
														<a href="./modify?id=${versionItem.id}">编辑</a>
														<a href="./delete?id=${versionItem.id}" onclick="return confirm('确认删除吗？')">删除</a>
													</td>
												</tr>
											</c:forEach>
										</tbody>
