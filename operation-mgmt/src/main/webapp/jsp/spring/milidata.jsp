...
<title>ZX_IMS 2.0 - 春节活动 米粒数据管理</title>
...
var previous = function(){
	...
	$.ajax({
		url : "${ctx}/spring/milidata/queryJson?mkey="+$("#mkey").val()+"&page="+page+"&pageSize="+pageSize,
		async : false,
		type : "GET",
...
var buildTableBody = function(dataList){
	...
	if(dataList && dataList.length > 0){
		for(var i=0;i<dataList.length;i++){
			...
			bodyHtml = bodyHtml + "</td>";
			bodyHtml = bodyHtml + "<td><a href=\"${ctx}/spring/milidata/f/"+dataList[i].id+"\">编辑</a></td>";
	...
	<section id="container" class="">
		...
		<section id="main-content">
			<section class="wrapper">
				<form id="form1" action="${ctx}/spring/milidata/query" method="post">
					<div class="row">
						<div class="col-lg-12">
							<section class="panel">
										...
										<tbody id="tbody">
											<c:forEach items="${dataObj.data.result}" var="item">
												<tr class="gradeX">
													...
													<td>
													<a href="${ctx}/spring/milidata/f/${item.id }">编辑</a>
													</td>
												</tr>
											</c:forEach>
										</tbody>
